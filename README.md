...
`market_data.bar_size` currently supports only `1 hour`, matching the fixed strategy model. The historical duration, IB `whatToShow`, RTH flag, candle-close buffer, gap backfill duration, and recent-gap block threshold are configured under `market_data`.
